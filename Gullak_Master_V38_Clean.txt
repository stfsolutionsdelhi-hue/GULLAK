...
function installAndRunDatabase() {
  ...
  // 3. Fix and align all sheets column formatting and headers
  autoFixAndAlignAllSheets(true);

  // 4. Secure sheets with warning protection
  enableSafeSheetProtection(true);

  // 5. Refresh Financials Summary
  syncFinancialsSheetBackend();
  SpreadsheetApp.flush();

  try {
    SpreadsheetApp.getUi().alert("✅ Database Successfully Initialized, Aligned & Secured!\n\n• All 14-Column Headers and Formats Aligned\n• Accidental Keyboard Entry Protection Enabled\n• Users & Financials Synchronized\n• Fully Ready for Web App & Sheet Operations!");
  } catch(e) {}
}
...
function autoFixAndAlignAllSheets(silent) {
  ...
  // 1. Fix and Intelligently Realign Members Sheet
  var memSheet = ss.getSheetByName("Members");
  if (memSheet) {
    var memH = ["Member ID", "Full Name", "Mobile Number", "Address", "Nominee / Ref", "RD / Month (₹)", "Status", "Date Joined", "Opening RD (₹)", "Due Day", "Custom Loan Limit (₹)", "Opening Loan (₹)", "Opening Int (₹)", "Opening Pen (₹)"];
    var lastR = memSheet.getLastRow();
    var lastC = Math.max(memSheet.getLastColumn(), memH.length);
    var cleanData = [];

    if (lastR > 1) {
      // Read all existing rows and columns to capture shifted / displaced cells
      var rawRows = memSheet.getRange(2, 1, lastR - 1, lastC).getValues();
      ...
      for (var i = 0; i < rawRows.length; i++) {
        ...
        var rdAmount = 0;
        var status = "ACTIVE";
        var dateJoined = "2026-01-01";
        var openingRd = 0;
        var dueDay = "15th of every month";
        var loanLimit = 0;
        var openLoan = 0;
        var openInt = 0;
        var openPen = 0;

        // 1. Detect Member ID (Check Col 0 or scan row)
        var c0 = String(row[0] || "").trim();
        if (/^(MBR|MEM|M|GUL)?\d+$/i.test(c0) || (c0.length > 0 && c0.length <= 15 && !/^[6-9]\d{9}$/.test(c0) && isNaN(Number(c0)))) {
          memberId = c0.toUpperCase();
        } else if (/^\d+$/.test(c0) && Number(c0) < 50000) {
          memberId = "MEM01012026" + c0;
        } else {
          memberId = "MEM01012026" + (i + 1);
        }

        // 2. Scan entire row for Status (ACTIVE / INACTIVE)
        for (var c = 0; c < row.length; c++) {
          var valStr = String(row[c] || "").trim().toUpperCase();
          if (valStr === "ACTIVE" || valStr === "INACTIVE") {
            status = valStr;
            break;
          }
        }

        // 3. Scan entire row for Mobile Number (10 digits starting with 6-9, or +91...)
        for (var c = 0; c < row.length; c++) {
          var valStr = String(row[c] || "").replace(/[^0-9]/g, "");
          if (valStr.length === 10 && /^[6-9]/.test(valStr)) {
            mobile = valStr;
            break;
          } else if (valStr.length === 12 && valStr.indexOf("91") === 0) {
            mobile = valStr.substring(2);
            break;
          }
        }
        if (!mobile && row[2]) {
        ...
        // 4. Scan entire row for Real Date (Year >= 2000)
        var foundDate = "";
        for (var c = 0; c < row.length; c++) {
          var cellVal = row[c];
          ...
          } else if (typeof cellVal === "string" && cellVal.trim().length > 0) {
            var s = cellVal.trim();
            if (/^\d{4}-\d{2}-\d{2}$/.test(s)) {
              var yr = parseInt(s.substring(0, 4), 10);
              if (yr >= 2000) { foundDate = s; break; }
        ...
        if (foundDate) {
          dateJoined = foundDate;
        }

        // 5. Scan entire row for RD / Month Amount (Detect 1900/1901 unformatted serial dates!)
        var foundRd = 0;
        for (var c = 0; c < row.length; c++) {
          var cellVal = row[c];
          if (cellVal instanceof Date) {
            var yr = cellVal.getFullYear();
            if (yr < 1970 && yr >= 1899) {
              // Extract original numeric amount from Google Sheets date serial
              var days = Math.round((cellVal.getTime() - new Date(1899, 11, 30).getTime()) / 86400000);
              if (days > 0 && days <= 50000) {
                foundRd = days;
                break;
              }
            }
          } else if (typeof cellVal === "number" && cellVal > 0) {
            if (cellVal >= 50 && cellVal <= 100000 && cellVal !== Number(mobile)) {
              if (c === 5 || c === 7 || c === 8 || foundRd === 0) {
                foundRd = cellVal;
              }
            }
          } else if (typeof cellVal === "string" && /^\d+$/.test(cellVal.trim())) {
            var num = parseInt(cellVal.trim(), 10);
            if (num >= 50 && num <= 100000 && cellVal.trim() !== mobile) {
              if (c === 5 || c === 7 || c === 8 || foundRd === 0) {
                foundRd = num;
              }
            }
        ...
        rdAmount = foundRd > 0 ? foundRd : 400;

        // 6. Detect Full Name
        if (row[1] && typeof row[1] === "string" && row[1].trim().length > 0) {
          var c1 = row[1].trim();
          if (c1.toUpperCase() !== "ACTIVE" && c1.toUpperCase() !== "INACTIVE" && !/^\d+$/.test(c1)) {
            fullName = c1;
          }
        }
        if (!fullName) {
          for (var c = 0; c < row.length; c++) {
            ...
            if (s.length > 2 && !/^\d+$/.test(s) && s.toUpperCase() !== "ACTIVE" && s.toUpperCase() !== "INACTIVE" && s !== dateJoined && s !== memberId && s.indexOf("-") !== 4) {
              fullName = s;
              break;
            }
          }
        }
        if (!fullName) fullName = "Member " + memberId;

        // 7. Detect Address and Nominee
        var textCandidates = [];
        for (var c = 0; c < row.length; c++) {
          var s = String(row[c] || "").trim();
          if (s.length > 0 &&
            ...
            if (!/^\d+(\.\d+)?$/.test(s) && s.toUpperCase() !== "ACTIVE" && s.toUpperCase() !== "INACTIVE") {
        ...
        if (textCandidates.length >= 2) {
          if (textCandidates[0].length >= textCandidates[1].length) {
            address = textCandidates[0];
            nominee = textCandidates[1];
          } else {
            nominee = textCandidates[0];
            address = textCandidates[1];
          }
          ...
          nominee = (row[4] ? String(row[4]).trim() : "-");
        }

        if (!address) address = "-";
        if (!nominee) nominee = "-";

        // 8. Due Day & Opening RD
        for (var c = 0; c < row.length; c++) {
          var s = String(row[c] || "").trim().toLowerCase();
          if (/^(\d{1,2}(st|nd|rd|th)?)$/i.test(s) || s.indexOf("every month") !== -1 || s.indexOf("due") !== -1) {
            dueDay = String(row[c]).trim();
            break;
          }
        }

        // Check if there is an Opening RD amount
    ...
    // Reset and format cleanly
    memSheet.clearContents();
    memSheet.clearFormats();
    memSheet.getRange(1, 1, 1, memH.length).setValues([memH]);
    memSheet.getRange(1, 1, 1, memH.length).setFontWeight("bold").setBackground("#1E293B").setFontColor("#FFFFFF");
    try { memSheet.setFrozenRows(1); } catch(e) {}

    if (cleanData.length > 0) {
      memSheet.getRange(2, 1, cleanData.length, memH.length).setValues(cleanData);
      memSheet.getRange(2, 1, cleanData.length, 5).setNumberFormat("@");
      memSheet.getRange(2, 6, cleanData.length, 1).setNumberFormat("0");
      memSheet.getRange(2, 7, cleanData.length, 1).setNumberFormat("@");
      memSheet.getRange(2, 8, cleanData.length, 1).setNumberFormat("yyyy-mm-dd");
      memSheet.getRange(2, 9, cleanData.length, 1).setNumberFormat("0");
      memSheet.getRange(2, 10, cleanData.length, 1).setNumberFormat("@");
      memSheet.getRange(2, 11, cleanData.length, 4).setNumberFormat("0");
    }
    alignAndFormatSheet(memSheet);
  }

  // 2. Fix Payments Sheet
  ...
  if (loanSheet) {
    ...
    if (lLast > 1) {
      var lRows = lLast - 1;
      loanSheet.getRange(2, 1, lRows, 1).setNumberFormat("@");
      loanSheet.getRange(2, 2, lRows, 1).setNumberFormat("yyyy-mm-dd");
      loanSheet.getRange(2, 3, lRows, 3).setNumberFormat("@");
      loanSheet.getRange(2, 6, lRows, 4).setNumberFormat("0");
      loanSheet.getRange(2, 10, lRows, 2).setNumberFormat("@");
    }
    alignAndFormatSheet(loanSheet);
  }

  // 4. Fix ExitSettlements
  ...
  if (bonusSheet) {
    ...
    alignAndFormatSheet(bonusSheet);
  }
  ...
  if (finSheet) {
    ...
    alignAndFormatSheet(finSheet);
  }

  SpreadsheetApp.flush();
  if (!silent) {
    try {
      SpreadsheetApp.getUi().alert("✅ All Sheets, Headers & Column Formats Successfully Fixed & Aligned!\n\n• Members Sheet: 14 Columns Header, RD (₹) Number, Date Joined\n• Payments, Loans, Settlements, Funds & Users Tab Standardized.");
    } catch(e) {}
  }
}
