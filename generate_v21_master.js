...
// We will construct the exact updated V21 PRO MASTER code.
// Let's ensure:
// 1. Users sheet created in installAndRunDatabase() with SANISH / Password and ADMIN / Admin@123.
// 2. getSocietyFullData() fetches users from "Users" sheet.
// 3. getCompleteSoftwareHtml() includes the Windows login modal on top of V21:
//    - NO Quick login button
//    - Has username, password with Eye toggle, Fullscreen button, Enter key support, Error toast / message.
//    - All V21 UI, tabs, modals, tables, scripts remain 100% untouched and functioning.
...
function installAndRunDatabase() {
  ...
  var loanH = ["Loan ID", "Date", "Member ID", "Name", "Type", "Principal (₹)", "Rate (%)", "Repaid (₹)", "Outstanding (₹)", "Status"];
  ...
  if (memSheet.getLastRow() <= 1) {
    var sampleM = [
      ["MEM0101261", "Rahul Kumar", "9810011111", "H-12, Sector 3, Rohini", "Sunita Kumar (Wife)", 400, "ACTIVE", "2026-01-01", 4800, "15th of every month", 0, 0, 0, 0],
      ["MEM0101262", "Suresh Sharma", "9810022222", "Shop 4, Main Market", "Vikas Sharma (Son)", 400, "ACTIVE", "2026-01-01", 4400, "15th of every month", 0, 0, 0, 0],
      ["MEM0101263", "Amit Verma", "9810033333", "B-45, Shastri Nagar", "Pooja Verma (Wife)", 400, "ACTIVE", "2026-01-01", 4400, "15th of every month", 0, 0, 0, 0],
      ["MEM0101264", "SANISH", "9718174244", "ASD", "DFFF", 400, "ACTIVE", "2026-01-01", 1000, "15th of every month", 0, 0, 0, 0]
    ];
...
function saveLoanBackend(l) {
  try {
      ...
      for (var i = 0; i < lIds.length; i++) {
        if (String(lIds[i][0]) === String(l.loanId)) {
          sheet.getRange(i + 2, 1, 1, 10).setValues([[
            ...
            Math.round(Number(l.repaid))||0, Math.round(Number(l.outstanding))||0, l.status||"ACTIVE"
          ]]);
          updated = true; break;
        }
      }
    ...
    if (!updated) {
      sheet.appendRow([l.loanId, formatPureDate(l.date), l.id, l.name, l.type||"Gullak Loan", Math.round(Number(l.principal))||0, Number(l.rate)||1.0, Math.round(Number(l.repaid))||0, Math.round(Number(l.outstanding))||l.principal, l.status||"ACTIVE"]);
    }
    SpreadsheetApp.flush();
    return { success: true };
  } catch (e) { return { success: false, error: e.toString() }; }
...
fs.writeFileSync("Part1_Server.gs", v21ServerPart, "utf8");
console.log("Part1_Server.gs written:", v21ServerPart.length);
