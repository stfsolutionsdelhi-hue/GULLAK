...
function getClientScriptPartB() {
  ...
  function bootApplication(){
    ...
    // STRICT DOUBLE-CONFIRMATION DELETE HANDLER (CROSS-REGISTER CASCADE)
    safeAddListener('btnDeleteMember', 'click', function(){
      ...
      var confirm1 = confirm('STEP 1 OF 2 - DELETE CONFIRMATION

Are you sure you want to delete member: "' + mName + '" (' + mid + ')?

This will remove this member from Master Ledger, Loan Register, Receipts, and Passbooks.');
      if(!confirm1) return;

      // Step 2: Second Strict Security Confirmation Prompt
      var confirm2 = prompt('STEP 2 OF 2 - FINAL PERMANENT DELETION CHECK
...
Type "DELETE" (in capital letters) to permanently erase all loans, receipts, and ledger history for "' + mName + '":');
      ...
      members = members.filter(function(x){ return String(x.id).trim().toUpperCase() !== cleanMid; });

      // 2. Remove all related Loans
      loans = loans.filter(function(l){ return String(l.id).trim().toUpperCase() !== cleanMid; });

      // 3. Remove all related Receipts / Payments
      payments = payments.filter(function(p){ return String(p.id).trim().toUpperCase() !== cleanMid; });

      // 4. Remove all related Exit Settlements
      if(typeof exitSettlements !== 'undefined' && exitSettlements){
        exitSettlements = exitSettlements.filter(function(e){ return String(e.id).trim().toUpperCase() !== cleanMid; });
      }

      // 5. Remove all related Bonus Settlements
      if(typeof bonusSettlements !== 'undefined' && bonusSettlements){
        bonusSettlements = bonusSettlements.filter(function(b){ return String(b.id).trim().toUpperCase() !== cleanMid; });
      }
