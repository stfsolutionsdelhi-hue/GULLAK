...
(function(){
  ...
  // CURRENT FINANCIAL YEAR ALWAYS VISIBLE + NEXT FINANCIAL YEAR AFTER FIRST ENTRY
  function setupFinancialYearDropdown(){
    var sel = document.getElementById("selFinancialYear");
    if(!sel) return;
    var curVal = sel.value || "FY 2026-27";
    
    var currentFY = "FY 2026-27";
    var fyMap = {};
    fyMap[currentFY] = true;

    function recordFYFromDate(dtStr){
      if(!dtStr) return;
      var dp = parseDateParts(dtStr);
      var startYr = (dp.mo >= 4) ? dp.yr : (dp.yr - 1);
      if(isNaN(startYr) || startYr < 2020 || startYr > 2100) return;
      var endYrShort = String(startYr + 1).substring(2);
      var fyKey = "FY " + startYr + "-" + endYrShort;
      fyMap[fyKey] = true;
    }

    payments.forEach(function(p){ recordFYFromDate(p.date); });
    loans.forEach(function(l){ recordFYFromDate(l.date); });
    exitSettlements.forEach(function(e){ recordFYFromDate(e.date); });
    bonusSettlements.forEach(function(b){ recordFYFromDate(b.date); });
    fundTransactions.forEach(function(f){ recordFYFromDate(f.date); });

    var sortedFys = Object.keys(fyMap).sort();
    var h = "";
    sortedFys.forEach(function(fy){
      var isSel = (fy === curVal || (curVal === "2026" && fy.indexOf("2026") >= 0)) ? "selected" : "";
      h += "<option value='" + fy + "' " + isSel + ">" + fy + "</option>";
    });
    sel.innerHTML = h;
  }
