# Build Part 3A: Script Engine
import os

script_3a = r'''function getClientScriptPartA() {
  return `
<script>
...
window.executeDirectLogin = function(e) {
  ...
  if (isSanish || isAdmin || matched) {
    ...
    var errMsg = "❌ <strong>Invalid Username or Password!</strong><br>Default: <strong>SANISH</strong> / <strong>Password</strong>";
    if (errBox) {
      errBox.innerHTML = errMsg;
      errBox.style.display = "block";
    }
    alert("❌ Login Failed: Username ya Password galat hai. Please check and try again.");
...
window.handleForgotCredentials = function() {
  alert("📧 PASSWORD RECOVERY DISPATCHED:\\n\\nLogin credentials have been registered for authorized administrator email:\\nstfsolutionsdelhi@gmail.com\\n\\nDefault Admin Access:\\n• Username: SANISH\\n• Password: Password\\n\\nAap Google Sheet ke 'Users' tab me jaakar bhi password change kar sakte hain.");
};
...
with open("Part3A.gs", "w", encoding="utf-8") as f:
    f.write(script_3a.strip() + "\n")
print("Part3A.gs created successfully.")
